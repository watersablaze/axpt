import { getTransitionRegistryEntry } from "./transitionRegistry";

import { getTransitionKey } from "./dossierApprovalGates";
...
export function getTransitionConsequences({
  ...
  const registryEntry = getTransitionRegistryEntry(transitionKey);
  ...
  if (fromState === "ESCROW_PENDING" && toState === "ESCROW_FUNDED") {
    return [
      {
        type: "ESCROW_FUNDING_RECORDED",
        label: "Escrow funding will be recorded",
        detail:
          "The dossier will move into escrow funded only after funding confirmation has been attached and activated.",
        severity: "INFO",
      },
      {
        type: "TREASURY_REVIEW_READY",
        label: "Treasury review may open next",
        detail:
          "After escrow funding is recorded, treasury review and authorization preparation can proceed.",
        severity: "WARNING",
      },
    ];
  }
