...
import { applyTreasuryTransferCapacityAssessmentIdempotentlyWithClient } from "../../treasury/gateway/transfers/application/applyTreasuryTransferCapacityAssessmentIdempotentlyWithClient";

type IdentityFactory = () => string;

export type ControlCenterApplyCapacityAssessmentHttpResult =
  | Readonly<{
      status: 200;
...
export async function applyTransferCapacityAssessmentHttp(params: {
  ...
  now?: () => Date;
}): Promise<ControlCenterApplyCapacityAssessmentHttpResult> {
  const {
    rawTransferId,
    rawAssessmentId,
    request,
  ...
  if (assessmentId.length === 0) {
    return {
      ...
      body: {
        ...
        error: "CAPACITY_ASSESSMENT_ID_REQUIRED",
      },
    };
  }
  ...
  const context = {
    commandId: `treasury-transfer-capacity-assessment-application-command-${generateIdentity()}`,

    actorId: principal.userId,

    correlationId: `treasury-transfer-capacity-assessment-application-${generateIdentity()}`,
  ...
  const eventId = `treasury-transfer-capacity-assessment-applied-event-${generateIdentity()}`;
  ...
  try {
    ...
    if (
      isErrorCode(error, "TRANSFER_CAPACITY_ASSESSMENT_TRANSFER_MISMATCH") ||
      isErrorCode(error, "TRANSFER_CAPACITY_ASSESSMENT_COMMAND_TARGET_MISMATCH")
    ) {
      return {
        status: 409,
        ...
        body: {
          ...
          error: "TREASURY_CAPACITY_ASSESSMENT_TARGET_MISMATCH",
        },
      };
    }
    ...
    /*
     * Another request may win persistence of the same
     * idempotency key after this request's initial lookup.
     *
     * Retry the unchanged application once so the durable
     * command receipt can resolve the accepted Transfer.
     */
      ...
      if (isErrorCode(error, "TRANSITION")) {
        return {
          ...
          body: {
            ...
            error:
              "TREASURY_CAPACITY_ASSESSMENT_APPLICATION_TRANSITION_INVALID",
          },
        };
      }
