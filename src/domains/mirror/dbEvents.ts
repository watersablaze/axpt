...
import { buildDoubleEntryRows } from '@/domains/mirror/ledgerWriter'
import { resolveLedgerAccountId } from '@/domains/mirror/ledgerAccounts'
import { decodeTokenType } from '@/domains/mirror/encoding'
...
export async function persistLogs(opts: PersistOpts) {
  ...
  for (const l of logs) {
    try {
      ...
      const normalizedTokenType = decodeTokenType(tokenType)
