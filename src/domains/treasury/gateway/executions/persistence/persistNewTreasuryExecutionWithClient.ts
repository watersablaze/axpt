...
export async function persistNewTreasuryExecutionWithClient(params: {
  ...
  if (aggregate.status !== TREASURY_EXECUTION_STATUS.CREATED) {
    throw new Error(
      `[TREASURY_GATEWAY_NEW_EXECUTION_STATUS_INVALID] ${aggregate.status}`,
    );
  }

  if (event.eventType !== TREASURY_EVENT_TYPE.TREASURY_EXECUTION_CREATED) {
    throw new Error(
      `[TREASURY_GATEWAY_NEW_EXECUTION_EVENT_INVALID] ${event.eventType}`,
    );
  }

  if (event.payload.executionId !== aggregate.id) {
    throw new Error(
      `[TREASURY_GATEWAY_EXECUTION_EVENT_TARGET_MISMATCH] ${event.payload.executionId} -> ${aggregate.id}`,
    );
  }
