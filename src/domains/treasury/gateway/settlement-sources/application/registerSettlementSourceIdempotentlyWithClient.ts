...
} from "../persistence/loadSettlementSourceWithClient";
...
import {
  registerSettlementSourceDurablyWithClient,
} from "./registerSettlementSourceDurablyWithClient";

const REGISTER_SETTLEMENT_SOURCE_COMMAND_KIND =
  "REGISTER_SETTLEMENT_SOURCE";

export type IdempotentSettlementSourceRegistrationDisposition =
  | "REGISTERED"
  | "REPLAYED";

export type IdempotentSettlementSourceRegistrationResult =
  Readonly<{
    ...
    disposition:
      IdempotentSettlementSourceRegistrationDisposition;
  }>;
...
function fingerprintRequest(
  request: RegisterSettlementSourceDurably,
): string {
...
export async function registerSettlementSourceIdempotentlyWithClient(
  params: {
    request: RegisterSettlementSourceDurably;

    client: TransactionClient;
  },
): Promise<IdempotentSettlementSourceRegistrationResult> {
  const {
    request,
    client,
  } = params;
  ...
  if (existingReceipt) {
    if (
      ...
      REGISTER_SETTLEMENT_SOURCE_COMMAND_KIND
    ) {
      throw new Error(
    ...
    const loaded =
      await loadSettlementSourceWithClient({
        settlementSourceId:
  ...
  const created =
    await registerSettlementSourceDurablyWithClient({
      request,
