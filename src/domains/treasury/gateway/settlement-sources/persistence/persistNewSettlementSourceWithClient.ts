...
export async function persistNewSettlementSourceWithClient(
  params: {
...
): Promise<PersistedNewSettlementSource> {
  const {
    result,
    eventId,
    context,
  ...
  if (
    ...
    throw new Error(
      `[TREASURY_GATEWAY_NEW_SETTLEMENT_SOURCE_STATUS_INVALID] ${aggregate.status}`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      `[TREASURY_GATEWAY_NEW_SETTLEMENT_SOURCE_EVENT_INVALID] ${event.eventType}`,
    );
  }

  if (
    event.payload.settlementSourceId !==
    aggregate.id
  ) {
    throw new Error(
      `[TREASURY_GATEWAY_SETTLEMENT_SOURCE_EVENT_TARGET_MISMATCH] ${event.payload.settlementSourceId} -> ${aggregate.id}`,
    );
  }
  ...
  try {
    ...
    const persistedEvent =
      await client.treasuryGatewayEvent.create({
        data: {
          eventId,
    ...
    return {
      ...
      event: {
        ...
        previousEventHash:
          persistedEvent.previousEventHash ??
          undefined,

        eventHash:
          persistedEvent.eventHash ??
          undefined,
      },
    };
  } catch (error: unknown) {
    if (
      ...
      if (target.includes("eventId")) {
        throw new Error(
          `[TREASURY_GATEWAY_EVENT_ID_CONFLICT] ${eventId}`,
        );
      }
