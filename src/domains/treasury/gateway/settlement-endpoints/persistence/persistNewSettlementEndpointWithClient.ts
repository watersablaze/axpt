...
export async function persistNewSettlementEndpointWithClient(
  params: {
  ...
  if (
    ...
    throw new Error(
      `[TREASURY_GATEWAY_NEW_SETTLEMENT_ENDPOINT_STATUS_INVALID] ${aggregate.status}`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      `[TREASURY_GATEWAY_NEW_SETTLEMENT_ENDPOINT_EVENT_INVALID] ${event.eventType}`,
    );
  }

  if (
    event.payload.settlementEndpointId !==
    aggregate.id
  ) {
    throw new Error(
      `[TREASURY_GATEWAY_SETTLEMENT_ENDPOINT_EVENT_TARGET_MISMATCH] ${event.payload.settlementEndpointId} -> ${aggregate.id}`,
    );
  }
