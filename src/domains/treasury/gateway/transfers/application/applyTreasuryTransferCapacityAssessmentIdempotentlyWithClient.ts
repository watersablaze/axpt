...
import { applyTreasuryTransferCapacityAssessmentDurablyWithClient } from "./applyTreasuryTransferCapacityAssessmentDurablyWithClient";

const APPLY_TREASURY_TRANSFER_CAPACITY_ASSESSMENT_COMMAND_KIND =
  "APPLY_TREASURY_TRANSFER_CAPACITY_ASSESSMENT";

export type IdempotentTreasuryTransferCapacityAssessmentApplicationDisposition =
  "APPLIED" | "REPLAYED";

export type IdempotentTreasuryTransferCapacityAssessmentApplicationResult =
  Readonly<{
    aggregate: TreasuryTransfer;

    receipt: TreasuryGatewayCommandReceipt;

    disposition: IdempotentTreasuryTransferCapacityAssessmentApplicationDisposition;
  }>;
...
export async function applyTreasuryTransferCapacityAssessmentIdempotentlyWithClient(params: {
  transferId: TreasuryTransferId;
...
}): Promise<IdempotentTreasuryTransferCapacityAssessmentApplicationResult> {
  ...
  if (existingReceipt) {
    if (
      ...
      APPLY_TREASURY_TRANSFER_CAPACITY_ASSESSMENT_COMMAND_KIND
    ) {
      throw new Error(
  ...
  const receipt = await persistTreasuryGatewayCommandReceiptWithClient({
    receipt: {
      ...
      commandKind: APPLY_TREASURY_TRANSFER_CAPACITY_ASSESSMENT_COMMAND_KIND,
