...
import type { PersistedTreasuryTransferTransition } from "./contracts";

function toJsonValue(value: unknown) {
  return JSON.parse(JSON.stringify(value));
}

export async function persistTreasuryTransferTransitionWithClient<
  TPayload,
>(params: {
  expectedVersion: number;

  result: TreasuryDomainResult<TreasuryTransfer, TPayload>;
  ...
  if (updated.count !== 1) {
    throw new Error(
      `[TREASURY_GATEWAY_TRANSFER_CONCURRENCY_CONFLICT] ${aggregate.id}@${expectedVersion}`,
    );
  }
