...
import type { PersistedTreasuryAllocationTransition } from "./contracts";

function toJsonValue(value: unknown) {
  return JSON.parse(JSON.stringify(value));
}

export async function persistTreasuryAllocationTransitionWithClient<TPayload>(
  params: {
    expectedVersion: number;

    result: TreasuryDomainResult<TreasuryAllocation, TPayload>;
  ...
  if (updated.count !== 1) {
    throw new Error(
      `[TREASURY_GATEWAY_ALLOCATION_CONCURRENCY_CONFLICT] ${aggregate.id}@${expectedVersion}`,
    );
  }
