...
import type { PersistedNewTreasuryAllocation } from "./contracts";
...
export async function persistNewTreasuryAllocationWithClient(params: {
  result: TreasuryDomainResult<
    TreasuryAllocation,
    TreasuryAllocationCreatedPayload
  >;
  ...
  if (aggregate.status !== TREASURY_ALLOCATION_STATUS.PROPOSED) {
    throw new Error(
      `[TREASURY_GATEWAY_NEW_ALLOCATION_STATUS_INVALID] ${aggregate.status}`,
    );
  }

  if (event.eventType !== TREASURY_EVENT_TYPE.TREASURY_ALLOCATION_CREATED) {
    throw new Error(
      `[TREASURY_GATEWAY_NEW_ALLOCATION_EVENT_INVALID] ${event.eventType}`,
    );
  }

  if (event.payload.allocationId !== aggregate.id) {
    throw new Error(
      `[TREASURY_GATEWAY_ALLOCATION_EVENT_TARGET_MISMATCH] ${event.payload.allocationId} -> ${aggregate.id}`,
    );
  }
