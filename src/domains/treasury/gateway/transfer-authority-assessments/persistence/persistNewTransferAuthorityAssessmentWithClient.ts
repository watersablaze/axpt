...
import type { PersistedNewTransferAuthorityAssessment } from "./contracts";
...
export async function persistNewTransferAuthorityAssessmentWithClient(params: {
  ...
  if (
    ...
    throw new Error(
      `[TREASURY_GATEWAY_NEW_TRANSFER_AUTHORITY_ASSESSMENT_EVENT_INVALID] ${event.eventType}`,
    );
  }

  if (event.payload.assessmentId !== aggregate.id) {
    throw new Error(
      `[TREASURY_GATEWAY_TRANSFER_AUTHORITY_ASSESSMENT_EVENT_TARGET_MISMATCH] ${event.payload.assessmentId} -> ${aggregate.id}`,
    );
  }

  if (event.payload.transferId !== aggregate.transferId) {
    throw new Error(
      `[TREASURY_GATEWAY_TRANSFER_AUTHORITY_ASSESSMENT_EVENT_TRANSFER_MISMATCH] ${event.payload.transferId} -> ${aggregate.transferId}`,
    );
  }
  ...
  try {
    await client.treasuryGatewayAggregate.create({
      data: {
        ...
        /*
         * Authority Assessments are immutable findings.
         * Their result is therefore their indexed aggregate posture.
         */
