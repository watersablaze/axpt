...
import type { PersistedTreasuryExecutionPlanTransition } from "./contracts";

function toJsonValue(value: unknown) {
  return JSON.parse(JSON.stringify(value));
}

export async function persistTreasuryExecutionPlanTransitionWithClient<
  TPayload,
>(params: {
  expectedVersion: number;

  result: TreasuryDomainResult<TreasuryExecutionPlan, TPayload>;
  ...
  if (updated.count !== 1) {
    throw new Error(
      `[TREASURY_GATEWAY_EXECUTION_PLAN_CONCURRENCY_CONFLICT] ${aggregate.id}@${expectedVersion}`,
    );
  }
